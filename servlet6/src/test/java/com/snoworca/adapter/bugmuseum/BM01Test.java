...
/**
 * BM01 — servlet6 mirror of servlet5 BugMuseum BM01 (FR-FIX-001).
 *
 * StreamConverter.convert(jakarta.servlet.ServletInputStream).read must delegate to the origin
 * inputstream.read, not to inputstream.readLine. Regression for jakarta 6.0
 * (adapter.servletElementConverter6.StreamConverter).
 *
 * @req FR-TEST-004
 */
class BM01Test {
    ...
    void shouldReproduceBug01() throws Exception {
        ...
        javax.servlet.ServletInputStream converted =
            adapter.servletElementConverter6.StreamConverter.convert(origin);
