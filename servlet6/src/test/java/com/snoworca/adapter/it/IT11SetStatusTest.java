 ...
 * on the javax surface. The jakarta side ({@code jakarta.servlet 6.0}) has REMOVED
 ...
 *     phrase MUST NOT surface on the HTTP status line (jakarta 6.0 removed the API; Tomcat 10.1
...
class IT11SetStatusTest {
    ...
    void shouldIgnoreReasonPhrase() throws Exception {
        try (TomcatRunner runner = new TomcatRunner()) {
            ...
            // jakarta.servlet 6.0 REMOVED setStatus(int, String); adapter (SRS §6.2.2 E1) forwards
            // to setStatus(int) only. Tomcat 10.1 emits the standard reason phrase or omits it
