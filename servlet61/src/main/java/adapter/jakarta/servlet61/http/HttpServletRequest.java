...
import adapter.jakarta.servlet61.ServletRequest;
...
public class HttpServletRequest extends ServletRequest implements jakarta.servlet.http.HttpServletRequest {
   ...
   @Override public boolean authenticate(jakarta.servlet.http.HttpServletResponse arg0) throws IOException, ServletException {
      try {
         return this.httpRequest.authenticate(new adapter.javax.servlet61.http.HttpServletResponse(arg0));
   ...
   @Override public <T extends HttpUpgradeHandler> T upgrade(Class<T> handlerClass) throws ServletException {
       try {
          adapter.javax.servlet61.http.HttpUpgradeHandler.setJakartaUpgradeHandlerClass(handlerClass);
          adapter.javax.servlet61.http.HttpUpgradeHandler handler = this.httpRequest.upgrade(adapter.javax.servlet61.http.HttpUpgradeHandler.class);
          jakarta.servlet.http.HttpUpgradeHandler  httpUpgradeHandler = new adapter.jakarta.servlet61.http.HttpUpgradeHandler(handler);
   ...
   // @Override removed: isRequestedSessionIdFromUrl() removed from jakarta.servlet.http.HttpServletRequest in 6.0
   // SRS §6.2.2 policy E — rewrite to isRequestedSessionIdFromURL (non-deprecated equivalent).
   // @req FR-CONV-010
