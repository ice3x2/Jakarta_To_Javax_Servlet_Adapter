...
import adapter.jakarta.servlet61.ServletResponse;
...
import java.util.Locale;
 ...
 public class HttpServletResponse extends ServletResponse implements jakarta.servlet.http.HttpServletResponse {
   ...
   // @Override removed: encodeUrl(String) removed from jakarta.servlet.http.HttpServletResponse in 6.0
   public String encodeUrl(String url) {
      return this.httpResponse.encodeURL(url);
   }

   // @Override removed: encodeRedirectUrl(String) removed from jakarta.servlet.http.HttpServletResponse in 6.0
   // SRS §6.2.2 policy E — rewrite to encodeRedirectURL (non-deprecated equivalent).
   // @req FR-CONV-010
   ...
   // SRS §6.2.2 policy E — Servlet 6.1 신규 오버로드를 javax 측에 동등 구성으로 재배치
   // (setStatus + Location header + 선택적 resetBuffer + flushBuffer).
   // @req FR-CONV-014
   @Override public void sendRedirect(String location, int sc, boolean clearBuffer) throws IOException {
      ...
      this.httpResponse.setHeader("Location", location);
      this.httpResponse.flushBuffer();
   }
   ...
   // @Override removed: setStatus(int, String) removed from jakarta.servlet.http.HttpServletResponse in 6.0
   // SRS §6.2.2 policy E1 — drop reason phrase, call setStatus(int) only.
   // @req FR-CONV-010
   ...
   @Override public void addCookie(jakarta.servlet.http.Cookie arg0) {
      this.httpResponse.addCookie(new adapter.javax.servlet61.http.Cookie(arg0));
   }
}
