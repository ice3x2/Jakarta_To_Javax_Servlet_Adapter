...
import adapter.common.HttpComponentConverter;

import adapter.javax.servlet61.ServletRequest;
import adapter.servletElementConverter61.AsyncContextConverter;
...
public class HttpServletRequest extends ServletRequest implements javax.servlet.http.HttpServletRequest {
   ...
   @Override public RequestDispatcher getRequestDispatcher(String path) {
      return new adapter.javax.servlet61.RequestDispatcher(this.httpRequest.getRequestDispatcher(path));
   }

   @SuppressWarnings("deprecation")
   @Override public String getRealPath(String path) {
      // placeholder pending PH-004 — getRealPath removed in Servlet 6.0
      return null;
   }
   ...
   @Override public ServletContext getServletContext() {
      return new adapter.javax.servlet61.ServletContext(this.httpRequest.getServletContext());
   }
   ...
   @SuppressWarnings("deprecation")
   @Override public boolean isRequestedSessionIdFromUrl() {
      // placeholder pending PH-004 — isRequestedSessionIdFromUrl removed in Servlet 6.0; fallback to URL variant
      return this.httpRequest.isRequestedSessionIdFromURL();
   }

   @Override public boolean authenticate(javax.servlet.http.HttpServletResponse response) throws IOException, ServletException {
      try {
         return this.httpRequest.authenticate(new adapter.jakarta.servlet61.http.HttpServletResponse(response));
   ...
   @Override public <T extends HttpUpgradeHandler> T upgrade(Class<T> handlerClass) throws ServletException {
      try {
         adapter.jakarta.servlet61.http.HttpUpgradeHandler.setJakartaUpgradeHandlerClass(handlerClass);
         adapter.jakarta.servlet61.http.HttpUpgradeHandler handler = this.httpRequest.upgrade(adapter.jakarta.servlet61.http.HttpUpgradeHandler.class);
         javax.servlet.http.HttpUpgradeHandler  httpUpgradeHandler = new adapter.javax.servlet61.http.HttpUpgradeHandler(handler);
