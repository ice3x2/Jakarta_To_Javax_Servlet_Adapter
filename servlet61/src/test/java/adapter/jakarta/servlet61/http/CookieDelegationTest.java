...
/**
 * jakarta-side {@link adapter.jakarta.servlet61.http.Cookie} 위임 단위 테스트.
 *
 * <p>본 클래스는 어댑터 Cookie 의 슈퍼클래스 ({@code jakarta.servlet.http.Cookie})
 * 중에서 어댑터가 명시적으로 {@code @Override} 한 위임 메서드 (setMaxAge, setSecure 등)
 * 가 원본 javax cookie 에 위임되는지 검증한다.
 *
 * <p>코드 사실 (2026-05-21 시점):
 * {@code adapter.jakarta.servlet61.http.Cookie} 는 jakarta 6.1 슈퍼클래스의
 * {@code setAttribute(String,String)} / {@code getAttribute(String)} 를 override
 * 하지 않는다. 따라서 attribute API 호출은 슈퍼클래스의 기본 구현 (jakarta 6.1 의
 * 내부 attribute map 저장) 으로 흐르며, 어댑터의 fallback 정책 (FR-CONV-013, no-op +
 * WARN-once) 은 javax-side 어댑터 ({@code adapter.javax.servlet61.http.Cookie}) 가
 * 단독 책임으로 cover 한다 (T-PH005-05 의 CookieAttributeFallbackTest 참조).
 *
 * <p>이 결정은 T-PH005-01 (servlet6 동일 패턴) 과 일관성을 유지하며, 본 어댑터가
 * "javax 4.0 wrap" 만 담당한다는 SRS 원칙에도 부합한다.
 *
 * @req FR-TEST-001
 */
