...
/**
 * jakarta-side {@link adapter.jakarta.servlet61.http.HttpServletResponse} 단위 위임 테스트.
 *
 * Mock 정책 (§0.6 / FR-TEST-001 AC-3): 원본 javax 인터페이스만
 * {@code withSettings().defaultAnswer(RETURNS_SMART_NULLS)} 로 mock.
 *
 * Note: servlet61 의 신규 sendRedirect(loc, sc, clearBuffer) edge case 검증은
 * sibling {@link HttpServletResponseRedirect61EdgeCaseTest} 가 담당한다.
 *
 * @req FR-TEST-001
 */
